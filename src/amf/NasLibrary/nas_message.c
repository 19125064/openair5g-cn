    ...
    size_t length)
{
  ...
  if (header->extended_protocol_discriminator == FIVEGS_MOBILITY_MANAGEMENT_MESSAGES || 
  	  header->extended_protocol_discriminator == FIVEGS_SESSION_MANAGEMENT_MESSAGES) {
    ...
    void *security)
{
  ...
  if (plain_msg) {
    ...
    if (size > 0) {
      ...
      #endif
	  OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS, "NAS message encrypt front: ", plain_msg, size)
    ...
    nas_message_decode_status_t * const status)
{
  ...
  if (plain_msg) {
    ...
    OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS, "NAS message decrypt front: ", buffer, length)
