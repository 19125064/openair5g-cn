  ...
  const allowed_nssai_t      const*  allowedNSSAI)
{
  ...
  AMF_APP_INITIAL_UE_MESSAGE(message_p).is_fiveG_s_tmsi_valid        = fiveG_s_tmsi == NULL? false: true;
  AMF_APP_INITIAL_UE_MESSAGE(message_p).is_amf_set_id_valid          = amfSetID     == NULL? false: true;
  AMF_APP_INITIAL_UE_MESSAGE(message_p).is_allowed_nssai_valid       = allowedNSSAI == NULL? false: true;
